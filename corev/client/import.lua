...
--- Create CoreV class
---@class corev_client
local corev_client = setmetatable({ __class = 'corev_client' }, {})

---@class corev_client_callback
corev_client.callback = setmetatable({ __class = 'corev_client_callback' }, {})
---@class corev_client_streaming
corev_client.streaming = setmetatable({ __class = 'corev_client_streaming' }, {})

--- Set default values for `corev-callback` class
corev_client.callback.requestId = 1
corev_client.callback.callbacks = {}

--- Return a value type of any CFX object
---@param value any Any value
---@return string Type of value
function corev_client:typeof(value)
...
function corev_client:toInt(value)
...
function corev_client:maxInt32(value)
...
function corev_client:ensure(input, defaultValue)
...
function corev_client:cfg(name, ...)
...
function corev_client:hashString(name)
    name = corev_client:ensure(name, 'unknown')
...
function corev_client:endswith(str, word)
    str = self:ensure(str, '')
...
function corev_client:replace(str, this, that)
...
function corev_client:split(str, delim)
    local t = {}
...
function corev_client:onServerTrigger(name, callback)
    name = corev_client:ensure(name, 'unknown')
    callback = corev_client:ensure(callback, function() end)
...
---@param name string Name of trigger
---@param callback function Trigger this function
function corev_client:onClientTrigger(name, callback)
    name = corev_client:ensure(name, 'unknown')
    callback = corev_client:ensure(callback, function() end)
...
function corev_client.callback:triggerCallback(name, callback, ...)
    name = corev_client:ensure(name, 'unknown')
    callback = corev_client:ensure(callback, function() end)
...
function corev_client.streaming:requestModelAsync(hash, cb)
    hash = corev_client:typeof(hash) == 'number' and hash or corev_client:ensure(hash, 'unknown')
    cb = corev_client:ensure(cb, function() end)

    if (corev_client:typeof(hash) == 'string') then
...
---@param textureDictionary string Name of texture dictonary to load
---@param cb function When dictonary is loaded, this function will be triggerd
function corev_client.streaming:requestTextureAsync(textureDictionary, cb)
    textureDictionary = corev_client:ensure(textureDictionary, 'unknown')
    cb = corev_client:ensure(cb, function() end)
...
corev_client:onServerTrigger(('corev:%s:serverCallback'):format(currentResourceName), function(requestId, ...)
    requestId = corev_client:ensure(requestId, 0)

    if (requestId <= 0 or requestId > 65535) then return end
    if (((corev_client.callback or {}).callbacks or {})[requestId] == nil) then return end

    corev_client.callback.callbacks[requestId](...)
    corev_client.callback.callbacks[requestId] = nil
end)

--- Returns stored resource name or call `GetCurrentResourceName`
---@return string Returns name of current resource
function corev_client:getCurrentResourceName()
...
function corev_client:getRandomString(length, recurse)
...
_G.string.startsWith = function(self, word)
    word = corev_client:ensure(word, 'unknown')
